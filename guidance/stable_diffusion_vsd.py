...
class StableDiffusionVSDGuidance(BaseGuidance):
    def __init__(self, cfg):
        ...
        while not ok:
            try:
                ...
                ok = True
                console.print("[red]ok in loading")
    ...
    def sample(
        ...
        **kwargs,
        ...
        text_embeddings_vd = prompt_utils.get_text_embedding(
            elevation,
            azimuth,
            camera_distances,
            use_view_dependent_prompt=self.cfg.use_view_dependent_prompt,
        )
    ...
    def sample_lora(
        ...
        **kwargs,
        ...
        text_embeddings = prompt_utils.get_text_embedding(
            elevation, azimuth, camera_distances, use_view_dependent_prompt=False
        )
    ...
    def forward(
        ...
        elevation: Float[Tensor, "B"],
        azimuth: Float[Tensor, "B"],
        camera_distance: Float[Tensor, "B"],
        c2w: Float[Tensor, "B 4 4"],
        ...
        **kwargs,
        ...
        # input text embeddings, view-independent
        text_embeddings = prompt_utils.get_text_embedding(
            elevation, azimuth, camera_distance, use_view_dependent_prompt=False
        )

        if self.cfg.camera_condition_type == "extrinsics":
            camera_condition = c2w.to(self.device)
            padding = torch.zeros_like(camera_condition[:, :1, :])
            padding[..., -1, -1] = 1.0
            camera_condition = torch.cat([camera_condition, padding], dim=1)
        elif self.cfg.camera_condition_type == "mvp":
            camera_condition = mvp_mtx
        else:
            raise ValueError(
        ...
        grad = self.compute_grad_vsd(
            ...
            text_embeddings_vd,
            text_embeddings,
            camera_condition.to(self.device),
        )
    ...
    def update(self, step: int):
        ...
        if self.cfg.grad_clip is not None:
            self.grad_clip_val = C(self.cfg.grad_clip, step)
